-- skript na vytvorenie tabuliek pre novy system global

-- hlavna tabulka
DROP TABLE IF EXISTS T_GLOBAL;
CREATE TABLE T_GLOBAL (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"PRODUCT_NR" TEXT,
	"DESCIPTION" TEXT
	"HOCHFLOR" INTEGER,
	"KNOTS" INTEGER,
	"WEIGHT" REAL,
	"BUY_PRICE" REAL,
	"COLOR" TEXT,
	"MATERIAL" TEXT,
	"MAT_INSIDE" TEXT,
	"FORM" TEXT,
-- tieto polozky budu v inych tabulkach
--	"SUPPLIER" TEXT,
--	"COEFFICIENT" REAL,
--	"STOCK" TEXT,
--	"COUNT" INTEGER,
	"COMMNET" TEXT,
	"VALID" INTEGER DEFAULT 1
);

-- sklady
DROP TABLE IF EXISTS T_STOCK;
CREATE TABLE T_STOCK (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"NAME" TEXT,
	"ADDRESS" TEXT,
	"ADDRESS2" TEXT,
	"EMAIL" TEXT,
	"PHONE" TEXT,
	"COMMENT" TEXT,
	"VALID" INTEGER DEFAULT 1
);

-- prepojenie sklad-global - stav kobercov na skladoch
DROP TABLE IF EXISTS T_STOCK_STATE;
CREATE TABLE T_STOCK_STATE (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"GLOBAL_ID" INTEGER,
	"STOCK_ID" INTEGER,
	"LENGTH" INTEGER,
	"WIDTH" INTEGER,
	"COUNT" INTEGER,
	"COMMENT" TEXT,
	"VALID" INTEGER DEFAULT 1
);

-- ciselne serie
DROP TABLE IF EXISTS T_NUMBER_SERIE;
CREATE TABLE T_NUMBER_SERIE (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"PREFIX" TEXT,
	"LAST_NR" INTEGER,
	"COMMENT" TEXT,
	"VALID" INTEGER DEFAULT 1
);

-- dodavatel
DROP TABLE IF EXISTS T_SUPPLIER;
CREATE TABLE T_SUPPLIER (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"NAME" TEXT,
	"ADDRESS" TEXT,
	"ADDRESS2" TEXT,
	"PHONE" TEXT,
	"EMAIL" TEXT,
	"NR_SERIE_ID" INTEGER,
	"COMMENT" TEXT,
	"VALID" INTEGER DEFAULT 1
);

-- odberatel
DROP TABLE IF EXISTS T_PURCHASER;
CREATE TABLE T_PURCHASER (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"NAME" TEXT,
	"ADDRESS" TEXT,
	"ADDRESS2" TEXT,
	"PHONE" TEXT,
	"EMAIL" TEXT,
-- typ odberatela - 0 velkoodberatel 1 maloodber
	"PURCHASER_TYPE" INTEGER DEFAULT 0,
	"COMMENT" TEXT,
	"VALID" INTEGER DEFAULT 1
);

-- faktura
DROP TABLE IF EXISTS T_INVOICE;
CREATE TABLE T_INVOICE (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"PURCHASER_ID" INTEGER,
	"INVOICE_DATE" TEXT,
	"COMMENT" TEXT,
	"VALID" INTEGER DEFAULT 1
);

-- polozka faktury
DROP TABLE IF EXISTS T_SOLD_ITEM;
CREATE TABLE T_SOLD_ITEM (
	"ID" INTEGER PRIMARY KEY AUTOINCREMENT,
	"STOCK_STATE_ID" INTEGER,
	"BUY_PRICE" REAL,
	"COUNT" INTEGER,
	"PRICE_COEF" REAL,
	"RESULT_PRICE" REAL,
	"INVOICE_ID" INTEGER,
	"COMMENT" TEXT,
	"VALID" INTEGER DEFAULT 1
);
